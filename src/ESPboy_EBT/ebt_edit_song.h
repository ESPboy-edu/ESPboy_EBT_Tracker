...
#define SONG_ITEM_PAGE_1	SONG_ITEM_NAME
#define SONG_ITEM_PAGE_2	SONG_ITEM_SQUEEZE

signed char cur_song = SONG_ITEM_NAME;
...
void ebt_edit_song_draw(void)
{
	...
	if (cur_song >= SONG_ITEM_PAGE_2) page = 1;

	set_font_color(COL_TEXT_DARK);
	set_back_color(COL_BACK);

	snprintf(buf, sizeof(buf), "%i/2", page + 1);
	...
	if (page == 0)
	{
		ebt_item_color(cur_song == SONG_ITEM_NAME);
		...
		ebt_item_color(cur_song == SONG_ITEM_AUTHOR);
	...
	{
		ebt_item_color(cur_song == SONG_ITEM_SQUEEZE);

		put_str(sx, sy, "SQUEEZE");

		sy += 2;

		ebt_item_color(cur_song == SONG_ITEM_EXPORT);
		...
		for (int ch = 0; ch < MAX_CHANNELS; ++ch)
		{
			...
			ebt_item_color(cur_song == (SONG_ITEM_PAN_CH1 + ch));
...
void ebt_edit_song_update(void)
{
	...
	if (!(pad&PAD_ACT))
	{
		...
		{
			if (cur_song < SONG_ITEM_PAGE_2)
			{
				cur_song = SONG_ITEM_PAGE_2;
			}
			else
			{
				cur_song = SONG_ITEM_PAGE_1;
			}
		}
	}
	else
	{
		if (pad_t&PAD_ACT)
		{
			...
			case SONG_ITEM_NAME:
				ebt_player_stop();
				ebt_ask_name("SONG NAME", song->name, ebt_edit_song_name_cb_ok, ebt_edit_song_cb_cancel);
				break;

			case SONG_ITEM_AUTHOR:
				ebt_player_stop();
				ebt_ask_name("SONG AUTHOR", song->author, ebt_edit_song_author_cb_ok, ebt_edit_song_cb_cancel);
				break;
			...
			case SONG_ITEM_EXPORT:
				ebt_player_stop();
				ebt_ask_name("EXPORT SONG", song_name, ebt_edit_song_export_cb_ok, ebt_edit_song_cb_cancel);
				break;
			}
		}
	}
